...
public class UnButton implements IMinecraft, IGui
{
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY)
    {
        ...
        if (animation != null)
        {
            size += animation.wrap(16);
            x -= animation.wrap(8); y -= animation.wrap(8);
            IFont.NORMAL.drawCenteredString(guiGraphics, displayName, x + (size / 2), (y + size) - 8, new Color(255, 255, 255, animation.wrap(255)));
        }
    ...
    public void mouseClicked(double mouseX, double mouseY, int action)
    {
        if (action == 0 && prevHover) this.action.run();
    }
